...
class LanguageBubbleChart extends Component {
  constructor() {
    super()
    this.state = {
      width: 800,
  ...
  componentDidMount = () => {
    // console.log(JSON.stringify(this.props.districtData));
    // let childrenData = { children: this.props.districtData.slice(0, 11) };
  ...
  render() {
    ...
    let childrenData = { children: this.props.districtData }
    let bubbles = d3.pack().size([this.state.width - 4, this.state.width - 4])
          ...
          {dataNodes.map((d, i) => (
            <g key={d.data.language + i} className="node" transform={this.transform(d)}>
              <circle
                ...
                fill={color(i)}
              />
              <text key={d.data.language + "text1"} fill="white" textAnchor="middle" dy=".2em" fontSize={d.r / 4}>
                {d.data.language}
              </text>
              <text key={d.data.language + "text2"} fill="white" textAnchor="middle" dy="1.3em" fontSize={d.r / 4}>
                {d.data.total}
              </text>
            </g>
          ))}
...
export default LanguageBubbleChart
